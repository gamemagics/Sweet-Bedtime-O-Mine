...
public class RandomDungeonGenerator : MonoBehaviour
{
    ...
    private void NewRoute(int x, int y, int routeLength, Vector2Int previousPos)
    {
        if (routeCount < maxRoutes)
        {
            ...
            while (++routeLength < maxRouteLength)
            {
                ...
                //Go Straight
                NewRouteWithDirection(ref routeUsed, ref x, ref y, routeLength, previousPos, xOffset, yOffset, roomSize);
                ...
                //Go left
                NewRouteWithDirection(ref routeUsed, ref x, y, routeLength, previousPos, xOffset, -yOffset, roomSize);
                // if (Random.Range(1, 100) <= deviationRate)
                // {
                //     if (routeUsed)
                //     {
                ...
                // }
                //Go right
                NewRouteWithDirection(routeUsed, x, y, routeLength, previousPos, -xOffset, yOffset, roomSize);
                // if (Random.Range(1, 100) <= deviationRate)
                // {
                //     if (routeUsed)
                //     {
    ...
    private void NewRouteWithDirection(ref bool routeUsed, ref int x, ref int y, int routeLength, Vector2Int previousPos, int xOffset, int yOffset, int roomSize)
    {
        if (Random.Range(1, 100) <= deviationRate)
        {
            if (routeUsed)
