...
public class RandomDungeonGenerator : MonoBehaviour
{
    ...
    private void NewRoute(int x, int y, int routeLength, Vector2Int previousPos)
    {
        if (routeCount < maxRoutes)
        {
            ...
            while (++routeLength < maxRouteLength)
            {
                ...
                //Go left
                NewRouteWithDirection(routeUsed, x, y, routeLength, previousPos, -yOffset, xOffset, roomSize);
                if (Random.Range(1, 100) <= deviationRate)
                {
                    if (routeUsed)
                    {
    ...
    private void NewRouteWithDirection(bool routeUsed, int x, int y, int routeLength, Vector2Int previousPos, int xOffset, int yOffset, int roomSize)
    {
        if (Random.Range(1, 100) <= deviationRate)
        {
            if (routeUsed)
