...
public class RandomDungeonGenerator : MonoBehaviour
{
    ...
    private void NewRouteWithDirection(ref bool routeUsed, ref int x, ref int y, int routeLength, ref Vector2Int previousPos, ref int xOffset, ref int yOffset, ref int roomSize)
    {
        if (Random.Range(1, 100) <= deviationRate)
        {
            if (routeUsed)
