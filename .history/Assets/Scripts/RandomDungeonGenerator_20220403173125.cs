...
public class RandomDungeonGenerator : MonoBehaviour
{
    ...
    private void NewRoute(int x, int y, int routeLength, Vector2Int previousPos)
    {
        if (routeCount < maxRoutes)
        {
            ...
            while (++routeLength < maxRouteLength)
            {
                ...
                //Go Straight
                NewRouteWithDirection(ref routeUsed, ref x, ref y, ref routeLength, ref previousPos, ref xOffset, ref yOffset, ref roomSize);
    ...
    private void NewRouteWithDirection(ref bool routeUsed, ref int x, ref int y, ref int routeLength, ref Vector2Int previousPos, ref int xOffset, ref int yOffset, ref int roomSize)
    {
        if (Random.Range(1, 100) <= deviationRate)
        {
            if (routeUsed)
