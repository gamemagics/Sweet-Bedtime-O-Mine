...
public class RandomDungeonGenerator : MonoBehaviour
{
    ...
    private void NewRouteWithDirection(ref bool routeUsed, ref int x, ref int y, ref int routeLength, ref Vector2Int previousPos, int xOffset, int yOffset, int roomSize)
    {
        if (Random.Range(1, 100) <= deviationRate)
        {
            if (routeUsed)
