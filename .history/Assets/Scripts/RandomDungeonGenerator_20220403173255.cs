...
public class RandomDungeonGenerator : MonoBehaviour
{
    ...
    private void NewRouteWithDirection(ref bool routeUsed, ref int x, ref int y, int routeLength, Vector2Int previousPos, int xOffset, int yOffset, ref int roomSize)
    {
        if (Random.Range(1, 100) <= deviationRate)
        {
            if (routeUsed)
