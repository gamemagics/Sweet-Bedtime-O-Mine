    ...
    private void SetTiles()
    {
        ...
        for (int xMap = bounds.xMin - 1; xMap <= bounds.xMax + 1; xMap++)
        {
            for (int yMap = bounds.yMin - 1; yMap <= bounds.yMax + 1; yMap++)
            {
                ...
                {
                    //top left tile
                    if (xMap == bounds.xMin - 1 && yMap == bounds.yMin - 1)
                    {
                        wallMap.SetTile(pos, botWallTile[0]);
                    }
                    //top right tile
                    else if (xMap == bounds.xMax + 1 && yMap == bounds.yMin - 1)
                    {
                        wallMap.SetTile(pos, botWallTile[1]);
                    }
                    else if (xMap == bounds.xMin + 1 && yMap == bounds.yMin - 1)
                    {
                        wallMap.SetTile(pos, botWallTile[2]);
                    }
                    else if (xMap == bounds.xMin - 1 && yMap == bounds.yMin + 1)
                    {
                        wallMap.SetTile(pos, topWallTile[0]);
                    }
                    else if (xMap == bounds.xMin + 1 && yMap == bounds.yMin + 1)
                    {
                        wallMap.SetTile(pos, topWallTile[2]);
                    }
                    else if (xMap == bounds.xMin - 1 && yMap == bounds.yMin)
                    {
                        wallMap.SetTile(pos, topWallTile[1]);
                    }
                    else if (xMap == bounds.xMin + 1 && yMap == bounds.yMin)
                    {
                        wallMap.SetTile(pos, topWallTile[3]);
                    }
                    else if (xMap == bounds.xMin && yMap == bounds.yMin - 1)
                    {
                        wallMap.SetTile(pos, botWallTile[1]);
                    }
                    else if (xMap == bounds.xMin && yMap == bounds.yMin + 1)
                    ...
                    else if (xMap == bounds.xMax - 1 && yMap == bounds.yMax - 1)
                    {
                        wallMap.SetTile(pos, botWallTile[1]);
                    }
                    else if (xMap == bounds.xMax + 1 && yMap == bounds.yMax - 1)
                    {
                        wallMap.SetTile(pos, botWallTile[3]);
                    }
                    else if (xMap == bounds.xMax - 1 && yMap == bounds.yMax + 1)
                    {
                        wallMap.SetTile(pos, topWallTile[1]);
                    ...
                    else if (xMap == bounds.xMax - 1 && yMap == bounds.yMax)
                    {
                        wallMap.SetTile(pos, topWall
                    // if (tileBelow != null)
                    // {
